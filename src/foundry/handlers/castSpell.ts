...
function stampCastTargets(message: unknown, data: unknown, targetTokenIds: string[]) {
  const sourceUpdate = { flags: { tablemate: { targetTokenIds } } }
...
async function withCastTargets<T>(targetTokenIds: string[], run: () => Promise<T>): Promise<T> {
  const hookId = Hooks.on('preCreateChatMessage', (message, data) =>
    stampCastTargets(message, data, targetTokenIds)
  )
  try {
    return await run()
  } finally {
