...
function isMentioned(text: string, user: WorldUser): boolean {
  ...
  try {
    return new RegExp(`(^|[^\\p{L}\\p{N}])@${escapeRegExp(needle)}([^\\p{L}\\p{N}]|$)`, 'iu').test(
      text
    )
  } catch {
...
// The speaker's portrait for the notification image. Derived the way the app
// derives it (SideMenu / characterCore): prototype-token art first, then the
// actor image. pushableArtUrl decides what can actually be sent.
function portraitUrl(msg: ChatMessageLike): string | undefined {
  const actorId = msg.speaker?.actor
  if (!actorId) return undefined
  const actor = game.actors.get(actorId)
  return pushableArtUrl(actor?.prototypeToken?.texture?.src ?? actor?.img)
}
...
export async function notifyChatMessage(message: unknown): Promise<void> {
  try {
    ...
    await deliverPush(config, {
      ...
      title: notificationTitle(senderName(msg)),
